                        ...
                        switch (opcaopacientes)
                        {
                            case 1:
                                try
                                {
                                    ...
                                    else
                                    {
                                        ...
                                        using (var contexto = new ConsultorioContext())
                                        {
                                ...
                                try
                                {
                                    if (!listaPacienteAgenda.PacienteTemAgendamento(removepaciente))
                                    {
                                        if (listaPacienteAgenda.RemovePaciente(removepaciente))
                                        {
                                            ...
                                            using (var contexto = new ConsultorioContext())
                                            {
                                                contexto.Pacientes.Remove(removepaciente);
                                                contexto.SaveChanges();
                                            }
                                        }
                                        else
                            ...
                            case 4:
                                ...
                                foreach (Paciente _paciente in listaPacienteAgenda.ListaDePacientes.OrderBy(o => (opcaopacientes == 3 ? o.Cpf : o.Nome)).ToList())
                                {
                                    ...
                                    foreach (Consulta consulta in listaPacienteAgenda.Agenda.Where(w => w.PacienteCPF == _paciente.Cpf).ToList())
                                    {
                                        Console.Write(" ".PadRight(14, ' '));
                                        Console.WriteLine("Agendado para: " + consulta.Data);
                                        Console.Write(" ".PadRight(14, ' '));
                                        Console.Write(consulta.HoraInicialDT.ToShortTimeString().PadLeft(4, ' ') + " às ");
                                        Console.WriteLine(consulta.HoraFinalDT.ToShortTimeString().PadLeft(4, ' ') + "  ");
                                    }
                                }
                                Console.WriteLine("Pressione qualquer tecla para continuar");
                                Console.ReadKey();
                    ...
                    while (!sairagenda)
                    {
                        ...
                        switch (opcaoagenda)
                        {
                            case 1:
                                ...
                                Consulta agendahorario = new Consulta(dataConsulta, horaInicial, horaFinal, strcpf);
                                ...
                                if (listaPacienteAgenda.PodeAgendar(agendahorario))
                                {
                                    if (listaPacienteAgenda.CheckHorario(agendahorario))
                                    {
                                        if (listaPacienteAgenda.AddAgendamento(agendapaciente, agendahorario))
                                        {
                                            ...
                                            using (var contexto = new ConsultorioContext())
                                            {
                                                contexto.Consultas.Add(agendahorario);
                                                contexto.SaveChanges();
                                            }
                                        }
                                        else
                            ...
                            case 2:
                                ...
                                if (removeconsulta != null && listaPacienteAgenda.RemoveAgendamento(removeconsulta))
                                {
                                    Console.WriteLine("Agendamento cancelado com sucesso!");
                                    using (var contexto = new ConsultorioContext())
                                    {
                                        contexto.Consultas.Remove(removeconsulta);
                                        contexto.SaveChanges();
                                    }
                                }
                                else
                            ...
                            case 3:
                                ...
                                else if (apreAgenda.Equals("P"))
                                {
                                    ...
                                    consultas.AddRange(listaPacienteAgenda.Agenda.Where(w => DateTime.Parse(w.Data) >= dtInicial && DateTime.Parse(w.Data) <= dtFinal).ToList());
                                }
                                ...
                                foreach (Consulta consulta in consultas)
                                {
                                    ...
                                    Console.Write(" ");
                                    Paciente _paciente = listaPacienteAgenda.GetPaciente(consulta.PacienteCPF);
                                    if (dataTemp != DateTime.Parse(consulta.Data))
                                    {
                                        Console.Write(consulta.Data.PadLeft(10, ' ') + " ");
                                        DateTime.TryParse(consulta.Data, out dataTemp); ;
                                    }
                                    else
                                    {
                                        Console.Write("           ");
                                    }
                                    Console.Write(consulta.HoraInicial.PadLeft(4, ' ') + " ");
                                    Console.Write(consulta.HoraFinal.PadLeft(4, ' ') + " ");
                                    Console.Write(consulta.TempoTS.Hours.ToString().PadLeft(2, '0') + ":" + consulta.TempoTS.Minutes.ToString().PadLeft(2, '0') + " ");
                                    if (_paciente == null)
                                    {
                                        Console.WriteLine("Pressione qualquer tecla para continuar");
                                        Console.ReadKey();
